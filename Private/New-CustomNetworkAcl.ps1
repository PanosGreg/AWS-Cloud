...
$NaclId = (Get-EC2NetworkAcl -Filter @{name='vpc-id' ; value = $VpcId}).NetworkAclId
...
try {Remove-EC2NetworkAclEntry @params -Egress $true} catch {$null}
try {Remove-EC2NetworkAclEntry @params -Egress $false} catch {$null}

New-EC2Tag -Resource $NaclId -Tag $Tags
